        ...
        setTour({
            ...
            itinerary: tour.itinerary.map((item, index) =>
                index === itineraryIndex
                    ? {
                          ...item,
                          itinerary_highlight: item.itinerary_highlight.filter(
        ...
        setTour({
            ...
            itinerary: tour.itinerary.map((item, iIndex) =>
                iIndex === itineraryIndex
                    ? {
                          ...item,
                          itinerary_highlight: item.itinerary_highlight.map(
                              (act, hIndex) =>
                                  hIndex === highlightIndex ? value : act
                          ),
                      }
                    : item
            ),
            ...
            <div className="w-full">
                <Input
                    error={errorFields?.has(
                        `itinerary.${itineraryIndex}.itinerary_highlight.${highlightIndex}`
                    )}
                    onChange={(e) =>
                        handleInput(
                            e.target.value,
                    ...
                    value={
                        tour.itinerary[itineraryIndex].itinerary_highlight[
                            highlightIndex
                        ]
                    }
                    type="text"
                    id="inputOverviewTitle"
                    placeholder="e.g., Tokyo's oldest Buddhist temple"
                />
                <ErrorMessage
                    type={`itinerary.${itineraryIndex}.itinerary_highlight.${highlightIndex}`}
                />
            </div>
            <button
                type="button"
