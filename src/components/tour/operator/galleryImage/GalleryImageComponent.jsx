    ...
    return (
        <div className="relative">
            ...
            <img
                ...
                src={
                    previews?.[index] ??
                    `${API_ENDPOINTS.IMAGE.URL}/${tour?.gallery_image?.[index]}`
                }
                className={`w-full object-cover rounded-lg shadow-lg hover:shadow-xl transition-shadow cursor-pointer ${isOpen || tour?.gallery_image?.[index] ? 'block' : 'hidden'}`}
            />
            <input
                type="file"
            ...
            <button
                type="button"
                id="remove-{{$index + 1}}"
                className="hidden absolute top-2 right-2 w-8 h-8 bg-red-500 text-white rounded-full items-center justify-center hover:bg-red-600 transition-colors"
            >
