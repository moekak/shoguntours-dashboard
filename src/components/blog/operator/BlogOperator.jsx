                        ...
                        <div className="container">
                              ...
                              <div className="mb-8 sticky">
                                    <Breadcrumbs aria-label="breadcrumb" className='text-xs' sx={{ fontSize: '0.75rem' }}>
                                          ...
                                          <Typography sx={{ color: 'text.primary', fontSize: '0.8rem' }}>Create New Post</Typography>
                                    </Breadcrumbs>
                                    <h1 className="text-2xl font-bold text-gray-800 mt-3">Create New Blog Post</h1>
                                    <p className="text-gray-600 my-2">Share your travel insights and stories with the world</p>
                                    {errors?.length > 0 && (
                                          <Alert
                                                variant="error"
                                                title={errorTitle}
                              ...
                              <div className="space-y-6">
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm p-6">
                                          ...
                                          <div className="space-y-5">
                                                <div>
                                                      <Label required={true} htmlFor="title" error={errorFields?.has("title")}>Blog Title</Label>
                                                      <Input value={blog?.title} name='title' type="text" id="title" placeholder="e.g., The Ultimate Guide to Tokyo's Hidden Gems" onChange={(e)=> handleInput(e)} error={errorFields?.has("title")}/>
                                                      <ErrorMessage type="title"/>

                                                </div>
                                                <div>
                                                      <Label required={true} htmlFor="subtitle" error={errorFields?.has("subtitle")}>Subtitle</Label>
                                                      <Input value={blog?.subtitle} name='subtitle' type="text" id="subtitle" placeholder="e.g., Discover secret spots locals don't want tourists to know" onChange={(e)=> handleInput(e)} error={errorFields?.has("subtitle")}/>
                                                      <ErrorMessage type="subtitle"/>
                                                </div>
                                                
                                                <div className='flex justify-between gap-5'>
                                                      <div className="flex-1">
                                                            <Label required={true} htmlFor="blog_category_id" error={errorFields?.has("blog_category_id")}>Category</Label>
                                                            <Select
                                                                  defaultValue={blog?.blog_category_id ? blog.blog_category_id: ""}
                                                                  name="blog_category_id"
                                                                  options={categories}
                                                                  placeholder="Choose a category"
                                                                  className="dark:bg-dark-900"
                                                                  onChange={(selectedOption) => handleSelect(selectedOption, "blog_category_id")}
                                                                  error={errorFields?.has("blog_category_id")}
                                                            />  
                                                ...
                                                <div>
                                                      ...
                                                      <TextArea
                                                            ...
                                                            onChange={(value)=> handleSelect(value, "meta_description")}
                                                            error={errorFields?.has("meta_description")}
                                                      />
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm p-6">
                                          ...
                                          <div className={` ${errorFields?.has("featured_image") ? "border-[#e7000b] hover:border-[#e7000b]" :"border-gray-300 hover:border-[#465fff]"} border-2 border-dashed  rounded-lg p-8 text-center flex items-center flex-col justify-center  transition-colors cursor-pointer h-[350px]  ${preview == "" && (blog?.featured_image == "") ? "block" : "hidden"}`}>
                                                <i className="fas fa-cloud-upload-alt text-5xl text-gray-400 mb-6"></i>
                                                <p className="text-gray-600 font-medium mb-2 text-lg">Upload featured image</p>
                                                <p className="text-sm text-gray-500 mb-4">Drag and drop or click to select</p>
                                                <p className="text-sm text-gray-500 mb-6">Recommended: 1200x630px (16:9 ratio)</p>
                                                <input type="file" id='featured_image' accept="image/*" className="hidden" onChange={(e)=> handleImageChange(e)} />
                                                <label 
                                          ...
                                          <div className={`preview_container border-2 border-dashed border-gray-300 rounded-lg p-5 text-center hover:border-[#465fff] transition-colors cursor-pointer h-[350px] ${preview !== "" || blog?.featured_image !== "" ? "block" : "hidden"}`}>
                                                <label htmlFor="featured_image" className="h-full block cursor-pointer">
                                                      {!(blog?.featured_image instanceof File) && blog?.featured_image  && (
                                                            <img src={`${API_ENDPOINTS.IMAGE.URL}/${blog?.featured_image}`} alt="" className="preview_src h-full w-full object-cover"/>
                                                      )}
                                                      {preview && (
                                                      ...
                                                      <div className="prose prose-lg max-w-none">
                                                            <ReactMarkdown 
                                                                  ...
                                                                  components={{
                                                                        ...
                                                                        h3: ({children}) => <h3 className="text-xl font-semibold text-gray-800 mb-2 mt-5">{children}</h3>,
                                                                        ...
                                                                        a: ({href, children}) => (
                                                                              <a href={href} className="text-[#465fff] hover:underline" target="_blank" rel="noopener noreferrer">
                                                                                    {children}
                                                                              </a>
                                                                        ),
                                                                        img: ({src, alt}) => (
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm p-6">
                                          <h2 className="text-xl font-semibold text-gray-800 mb-6 flex items-center gap-2">
                                                <div className="w-8 h-8 bg-[#ecf3ff] rounded-lg flex items-center justify-center mr-3">
                                                      <LocalOfferOutlinedIcon sx={{ color: '#465fff', fontSize: 20 }} />
                                                </div>
                                                Tags & SEO
                                          </h2>
                                          ...
                                          <div className="space-y-5">
                                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                      <div>
                                                            <Label required={true} htmlFor="tags" error={errorFields?.has("tags")}>Tags</Label>
                                                            <Input value={blog?.tags} name='tags' type="text" id="tags" placeholder="tokyo, travel-tips, japan, culture (separate with commas)" onChange={(e)=> handleInput(e)} error={errorFields?.has("tags")}/>
                                                            <ErrorMessage type="tags"/>
                                                            <p className="text-xs text-gray-500 mt-1">Separate tags with commas</p>
                                                      </div>

                                                      <div>
                                                            <Label required={true} htmlFor="reading_time" error={errorFields?.has("reading_time")}>Reading time</Label>
                                                            <Input value={blog?.reading_time} name='reading_time' type="number" id="reading_time" placeholder="5" onChange={(e)=> handleInput(e)} error={errorFields?.has("reading_time")}/>
